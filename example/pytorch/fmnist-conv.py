...
if __name__ == "__main__":
    ...
    for i in range(10):
        ...
        plt.title(f"True: {FMNIST_LABELS[test_labels[i]]}, Predicted: {FMNIST_LABELS[predicted_classes[i]]}", color=color)
        plt.show()
    ...
    with torch.no_grad():
        ...
        plt.savefig(os.path.join(OUTPUT_DIR, "pytorch", "fmnist-cnn-error.png"))
    ...
    for img_path in PREDICT_IMAGES:
        ...
        with torch.no_grad():
            print(f"{img_path} is predicted as {FMNIST_LABELS[torch.max(model(img), 1)[1].item()]}")
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(OUTPUT_DIR, "pytorch", "fmnist-cnn-loss.png"))
    plt.show()
