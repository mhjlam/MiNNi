...
def main():
    ...
    with torch.no_grad():
        ...
        disp = metrics.ConfusionMatrixDisplay(confusion_matrix=cm, display_labels=example.FMNIST_LABELS.values())

        # Plot confusion matrix
        plt.figure(figsize=(10, 8))
        disp.plot(cmap="Blues", values_format="d")
        plt.title("Confusion Matrix")
        plt.savefig(os.path.join(example.OUTPUT_DIR, "pytorch_fmnist-dense-error.png"))
    ...
    plt.tight_layout()
    plt.savefig(os.path.join(example.OUTPUT_DIR, "pytorch_fmnist-dense-loss.png"))
    plt.show()
