...
MODEL_PATH = os.path.join(OUTPUT_DIR, "fmnist.minni")
...
def train(X, y, plot=False):
    ...
    if plot:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "multiclass", "fmnist-loss.png"))
        plt.show()
...
def evaluate(Xt, yt, plot=False):
    ...
    if plot:
        ...
        plt.tight_layout()
        plt.savefig(os.path.join(OUTPUT_DIR, "multiclass", "fmnist-error.png"))
        plt.show()
...
def predict(Xt, yt, images):
    ...
    for image in images:
        ...
        print(f"{image} is predicted as a {FMNIST_LABELS[yhat[0]]}")


if __name__ == "__main__":
    X, y, Xt, yt = preprocess_dataset()

    train(X, y, plot=False)
    evaluate(Xt, yt, plot=False)
    predict(Xt, yt, PREDICT_IMAGES)
